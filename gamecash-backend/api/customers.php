...
class CustomersAPI {
    ...
    public static function create($db, $data) {
        ...
        // Check uniqueness
        $check_query = "SELECT id FROM customers WHERE name = :name LIMIT 1";
        ...
        $query = "INSERT INTO customers (name, phone, total_debt) VALUES (:name, :phone, 0.00)";
        $stmt = $db->prepare($query);
    ...
    public static function update($db, $data) {
        ...
        // Check uniqueness
        $check_query = "SELECT id FROM customers WHERE name = :name AND id != :id LIMIT 1";
        $check_stmt = $db->prepare($check_query);
        $check_stmt->bindParam(":name", $name);
    ...
    public static function logPayment($db, $data) {
        ...
        try {
            ...
            $cust_query = "SELECT name, total_debt FROM customers WHERE id = :customer_id FOR UPDATE";
            ...
            $up_query = "UPDATE customers SET total_debt = :new_debt WHERE id = :customer_id";
            ...
            $pay_query = "INSERT INTO customer_payments (customer_id, amount_paid, notes) 
                          VALUES (:customer_id, :amount_paid, :notes)";
